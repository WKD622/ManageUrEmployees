...
def test_todays_events(client):
    # given
    number_of_todays_events = 3
    number_of_past_events = 4
    PastEventFactory.create_batch(number_of_past_events)
    expected_todays_events = TodaysEventsFactory.create_batch(number_of_todays_events)
    ...
    for x in json.loads(response.content):
        todays_events.add(x['id'])

    # then
    assert todays_events == {x.id for x in expected_todays_events}
